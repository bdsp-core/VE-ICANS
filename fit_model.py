...
class MyLogisticRegression(LogisticRegression):
    ...
    def __init__(self, class_weight=None, tol=1e-6,
            ...
            random_state=None, max_iter=1000, decimal_max_iter=1000,
            ...
            if np.sum(np.abs(coef_)>1e-5)==0:
                ...
                for _ in range(self.decimal_max_iter):
                    ...
                    if self.coef_sum is not None and np.abs(np.sum(new_coef*sum_weights)-self.coef_sum)>self.coef_decimal/10.:
                        continue
                    func_value = func(new_coef, X2, y2, alpha, self.l1_ratio, sample_weight)#np.r_[new_coef,intercept_]
...
class LTRPairwise(BaseEstimator, ClassifierMixin):
    ...
    """
    def __init__(self, estimator, classes=None,
                    class_weight=None, min_level_diff=0, verbose=True):
        super().__init__()
    ...
    def _generate_pairs(self, X, y, sample_weight):
        ...
        for i, j in combinations(range(len(X)), 2):
            ...
            if y[i] == y[j]:
                continue
            X2.append( X[i]-X[j] )
            y2.append( 1 if y[i]>y[j] else 0 )#TODO use self.min_level_diff?
            if sample_weight is not None:
                sw2.append( max(sample_weight[i], sample_weight[j]) )
        return np.array(X2), np.array(y2), np.array(sw2)
...
def get_sample_weights(y, class_weight='balanced', prior_count=0): #class_weight = 'None'
...
def get_perf(model_type, y, yp, yp_prob=None):
    if model_type=='logreg':
        perf = roc_auc_score(y, yp_prob[:,1])
    elif model_type=='ltr':
        perf = pearsonr(y, yp)[0]
...
def get_coef(model_type, model):
    if model_type=='logreg':
        ...
        coef = model.estimator.coef_.flatten()
    return coef
...
def fit_model(X, y, sids, cv_split_, bounds, orders, worst_delirium_mask, model_type='logreg', refit=True, n_jobs=-2, best_params=None, random_state=None):
    """
    """
    models = []
    cv_tr_score = []
    ...
    for cvi, cv_sids in enumerate(cv_split):#tqdm
        print("\nProcessing CV layer %d/%d" % (cvi+1, Ncv))
        ...
        if model_type=='logreg':
            ...
            model_params = {
                'estimator__C':[1],#np.logspace(-30,-5,5),#[:2]
                #'estimator__l1_ratio':np.arange(0.005,0.01,0.05,),#[:2]
                #'estimator__max_iter':[10,30,70,100,] #850, 950] # maybe 750 - 1000?
                }
            ...
            model = LTRPairwise(MyLogisticRegression(
                                    ...
                                    bounds=bounds,
                                    orders=orders), 
                                classes, class_weight='balanced', min_level_diff=2, #This line changing classes = None, class_weight = None, min_level_diff=0
                                verbose=1)
        ...
        #model = MyCalibrator(model)   # is this necessary?
        #model.fit(Xtr, ytr)
        
        zptr = np.zeros(len(ytr_full))+worst_delirium_label
        zptr[good_ids] = model.predict(Xtr)
        yptr_prob = np.zeros((len(ytr_full), n_classes))
        yptr_prob[:,-1] = 1
        yptr_prob[good_ids] = model.predict_proba(Xtr)
        
        models.append(model)
        cv_tr_score.append(get_perf(model_type, ytr_full, zptr))
        ...
        if len(cv_sids)>0:
            ...
            #ypte_z = model.base_estimator.predict(Xte2)#
            #ypte_z = np.zeros(len(yte_full))+np.inf
            ...
            zpte = np.zeros(len(yte_full)) + worst_delirium_label
            zpte[good_ids] = model.predict(Xte)
...
# =============================================================================
#             df_te = pd.DataFrame(
#                         data=np.c_[yte_full, zpte],
#                         columns=['y', 'z'])
# =============================================================================
            df_te = pd.DataFrame(
                        data=np.c_[yte_full, zpte, ypte_prob],
                        columns=['y', 'z',]+['prob(%d)'%k for k in classes])
            df_te['SID'] = sids_te
            y_yp_te.append(df_te)
            cv_te_score.append(get_perf(model_type, yte_full, zpte))
    ...
    if refit:
        ...
        #yp_z = model.base_estimator.predict(X2)#
        #yp_z = np.zeros(len(y_full))+np.inf
        ...
        zp = np.zeros(len(y_full)) + worst_delirium_label
        zp[good_ids] = model.predict(X)
...
# =============================================================================
#         df = pd.DataFrame(
#                     data=np.c_[y_full, zp],
#                     columns=['y','z'])
# =============================================================================
        df = pd.DataFrame(
                    data=np.c_[y_full,  zp, yp_prob], #yp_z,
                    columns=['y','z', ]+['prob(%d)'%k for k in classes]) #'yp'
        df['SID'] = sids
        y_yp_te.append(df)
...
if __name__=='__main__':
    ...
    y = dfy[yname].values.astype(int) # TODO temp fix was float
    ...
    # remove features with small std
    std_thres = 0.01
    ...
    orders = [
        #(list(Xnames).index('alpha >=10Hz'), list(Xnames).index('alpha 8<=9Hz')),
        #(list(Xnames).index('alpha 8<=9Hz'), list(Xnames).index('alpha <=8Hz')),
        #(list(Xnames).index('alpha <=8Hz'), list(Xnames).index('theta 6<=8Hz')),
        (list(Xnames).index('theta 6<=8Hz'), list(Xnames).index('theta 5<=6Hz')),
        (list(Xnames).index('theta 5<=6Hz'), list(Xnames).index('theta 4<=5Hz')),
        (list(Xnames).index('theta 4<=5Hz'), list(Xnames).index('delta 3<=4Hz')),
        (list(Xnames).index('delta 3<=4Hz'), list(Xnames).index('delta 2<=3Hz')),
        (list(Xnames).index('delta 2<=3Hz'), list(Xnames).index('delta 1<=2Hz')),
        (list(Xnames).index('delta 1<=2Hz'), list(Xnames).index('delta <=1Hz')),
# =============================================================================
#         (list(Xnames).index('theta slowing'), list(Xnames).index('theta > delta slowing')),
#         (list(Xnames).index('theta > delta slowing'), list(Xnames).index('theta = delta slowing')),
#         (list(Xnames).index('theta = delta slowing'), list(Xnames).index('delta > theta slowing')),
#         (list(Xnames).index('delta > theta slowing'), list(Xnames).index('delta slowing')),
#         
#         #(list(Xnames).index('f2 7<=8Hz'), list(Xnames).index('f2 6<=7Hz')),
#         (list(Xnames).index('f2 6<=7Hz'), list(Xnames).index('f2 5<=6Hz')),
#         (list(Xnames).index('f2 5<=6Hz'), list(Xnames).index('f2 4<=5Hz')),
#         (list(Xnames).index('f2 4<=5Hz'), list(Xnames).index('f2 3<=4Hz')),
#         (list(Xnames).index('f2 3<=4Hz'), list(Xnames).index('f2 2<=3Hz')),
#         (list(Xnames).index('f2 2<=3Hz'), list(Xnames).index('f2 1<=2Hz')),
#         (list(Xnames).index('f2 1<=2Hz'), list(Xnames).index('f2 <=1Hz')),
#         
#         (list(Xnames).index('f1 7<=8Hz'), list(Xnames).index('f1 6<=7Hz')),
#         (list(Xnames).index('f1 6<=7Hz'), list(Xnames).index('f1 5<=6Hz')),
#         (list(Xnames).index('f1 5<=6Hz'), list(Xnames).index('f1 4<=5Hz')),
#         (list(Xnames).index('f1 4<=5Hz'), list(Xnames).index('f1 3<=4Hz')),
#         (list(Xnames).index('f1 3<=4Hz'), list(Xnames).index('f1 2<=3Hz')),
#         (list(Xnames).index('f1 2<=3Hz'), list(Xnames).index('f1 1<=2Hz')),
#         (list(Xnames).index('f1 1<=2Hz'), list(Xnames).index('f1 <=1Hz')),
# =============================================================================
...
#     df_cv.SID = df_cv.SID.astype('str')
...
# =============================================================================
    ...
    ## Create another cv_split that works for continuous y
    test = sids.copy()
    #np.random.seed(random_state)
    #np.random.shuffle(test)
    #print('sids shuffled')
    #cv_split = np.array_split(test, Ncv) # [list(chunks(test_y, len(test_y)/Ncv))]
    cv = GroupKFold(n_splits=Ncv)
    cv_split = [test[teid] for trid, teid in cv.split(X, y, test)]
    
    # fit model with bootstrap
    print("Fitting model with bootstrap")
    tr_scores_bt = []
    te_scores_bt = []
    y_yp_bt = []
    coefs_bt = []
    ...
    for bti in tqdm(range(Nbt+1)):
        print("bootstrap loop ", bti)
        if bti==0:
            ybt = y
            Xbt = X
            sidsbt = sids
        else:
            #print("random state is ", np.random.get_state() )
            np.random.seed(bti)
            btids = np.random.choice(len(X), size=len(X), replace=True)#np.random.choice(len(X), len(X), replace=True)
            ybt = y[btids]
            print(btids)
        ...
        models, params, cv_tr_score, cv_te_score, y_yp = fit_model(
                            ...
                            best_params=params, # classes = classes,
                            random_state=random_state)
        ...
        if bti==0:
            ...
            df_coefs = pd.DataFrame(data={'name':Xnames[~np.in1d(Xnames, worst_delirium_Xnames)], 'coef':coefs_bt[-1]})
            print(df_coefs.sort_values('coef')[::-1])
    if Nbt>0:
        print('\ntr score: %f [%f -- %f]'%(tr_scores_bt[0], np.percentile(tr_scores_bt[1:], 2.5), np.percentile(tr_scores_bt[1:], 97.5)))
    ...
    ## Run Corellation test
    worst_score = 20
    h,w = X.shape
# =============================================================================
#     cfs = []
#     cdic = dict(zip(df_coefs.name, df_coefs.coef))
#     for c in dfX.columns[2:]:
#         if c in cdic:
#             cfs.append(cdic[c])
#         else: cfs.append(0)
#     cfs = np.array(cfs)
# =============================================================================
    cfs = np.pad(coefs_bt[0], (0, w-coefs_bt[0].shape[0]), 'constant')
    print("Using coefficients:", cfs)
    pred_scores = X @ cfs
    icans = dfy['icans'].values.astype(float)
    df_preds = pd.DataFrame()
    df_preds['icans']=pd.Series(icans)
    df_preds['score']=pd.Series(pred_scores)
    for i,r in dfX.iterrows():
        for n in worst_delirium_Xnames:
            if r[n] == 1:
                df_preds.loc[i, 'score'] = worst_score # What to set max to???
                
                
    print("pearsons:", df_preds.corr(method="pearson").iloc[0,1])
    print("spearmans:", df_preds.corr(method="spearman").iloc[0,1])
    
    #print("\nThere were %d total warnings" % total_warnings)
    
    df_preds = pd.concat([df_preds, dfX], axis=1)
    
# =============================================================================
#     with pd.ExcelWriter(f"pred_scores_{yname}_withProba.xlsx") as writer:  
#         df_preds.to_excel(writer, sheet_name='Results', index=False)
#         df_coefs.to_excel(writer, sheet_name='coefs', index=False)
# =============================================================================
